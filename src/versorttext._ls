;;; versorttext.lsp
;;; Copyright 1999-2005 by Lance A. Endres
...
(defun VL-VERSORTTEXT (SS2 / ADDTEMPARRAY CNT LOOP1 SS1 SS1LEN TEMP TEMPYPNT TEMP2 TEMP2YPNT)
    ...
    (while (and (= LOOP1 "True") (> SS1LEN 1))
	(setq SS1      SS2
	      SS2      (EST-VL-SS-BY-NAME "VERTSORTTEXT")
	      SS1LEN   (vla-get-count SS1)
	      LOOP1    NIL
	      CNT      0
	      TEMP     (vla-item SS1 CNT)
	      TEMPYPNT (vlax-safearray-get-element (vlax-variant-value (vla-get-insertionpoint TEMP)) 1)
	) ;_ End setq
	(while (< CNT (1- SS1LEN))
	    (setq CNT	    (1+ CNT)
		  TEMP2	    (vla-item SS1 CNT)
		  TEMP2YPNT (vlax-safearray-get-element (vlax-variant-value (vla-get-insertionpoint TEMP2)) 1)
	    ) ;_ End setq
	    (cond
		((<= TEMP2YPNT TEMPYPNT)
